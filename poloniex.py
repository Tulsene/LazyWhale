...
def createTimeStamp(datestr, format="%Y-%m-%d %H:%M:%S"):
    # strptime: Parse a string representing a time according to a format. The return value is a struct_time
    # mktime: returns a floating point number, for compatibility with time()
...
class Poloniex:
                    ...
                    if (isinstance(after['return'][x], dict)):
                        if ('datetime' in after['return'][x] and 'timestamp' not in after['return'][x]):
                            after['return'][x]['timestamp'] = \
                                float(createTimeStamp(after['return'][x]['datetime']))
    ...
    def api_query(self, command, req={}):
        ...
        if (command == "returnTicker" or command == "return24hVolume"):
            ...
            ret = urllib2.urlopen(urllib2.Request('https://poloniex.com/public?command=' \
            ...
            reqUrl = 'https://poloniex.com/public?command=' + "returnLoanOrders" + \
    ...
    def createLoanOffer(self, currency, amount, duration, autoRenew, lendingRate):
        return self.api_query('createLoanOffer', {"currency": currency, "amount": amount, \
                                                  "duration": duration, "autoRenew": autoRenew,
                                                  "lendingRate": lendingRate, })

    # Cancels an order you have placed in a given market. Required POST parameters \
    # are "currencyPair" and "orderNumber".
    # Inputs:
    # currencyPair  The currency pair
    ...
    def cancel(self, currencyPair, orderNumber):
        return self.api_query('cancelOrder', {"currencyPair": currencyPair, \
                                              "orderNumber": orderNumber})

    def cancelLoanOffer(self, currency, orderNumber):
        return self.api_query('cancelLoanOffer', {"currency": currency, \
                                                  "orderNumber": orderNumber})

    # Immediately places a withdrawal for a given currency, with no email confirmation. \
    # In order to use this method, the withdrawal privilege must be enabled for your \
    # API key. Required POST parameters are "currency", "amount", and "address". \
    ...
    def withdraw(self, currency, amount, address):
        return self.api_query('withdraw', {"currency": currency, "amount": amount, \
                                           "address": address})
