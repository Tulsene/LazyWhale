...
class Lazy:
    ...
    def __init__(self):
        self.buy_orders = []
        self.sell_orders = []
        self.currency_pair = 'BTC_SC'
        self.buy_pair = 'BTC'
        self.sell_pair = 'SC'
        self.amount = Decimal('10000.00000000')
        self.increment = Decimal('0.00000005')
        self.buy_price_min = Decimal('0.00000045')
        self.buy_price_max = Decimal('0.0000005')
        self.sell_price_min = Decimal('0.0000008')
        self.sell_price_max = Decimal('0.0000009')
        self.nb_orders_to_display = Decimal('1')  # Have to be a int entry
    ...
    def check_if_no_orders(self):
        ...
        """
        logging.debug('check_if_no_orders(self):')
    ...
    def update_sell_orders(self, buy_orders_missing, sell_orders_executed):
        ...
        """
        logging.debug('update_sell_orders(self):')
    ...
    def update_buy_orders(self, sell_orders_missing, buy_orders_executed):
        ...
        """
        logging.debug('update_buy_orders(self):')
    ...
    def limit_nb_orders_displayed(self):
        ...
        """
        logging.debug('limit_nb_orders_displayed(self):')
        new_buy_orders, new_sell_orders = self.remove_orders_off_strat()
                ...
                # Add sell orders if there is less than nb_orders_to_display
                    ...
                    if new_sell_orders[0][2] + self.nb_orders_to_display \
                        ...
                        i = int((self.sell_price_max - new_sell_orders[-1][2]) \
                                / self.increment)
                        logging.info('Sell price max almost reached')
        ...
        if self.buy_orders == []:
            ...
            log = 'Buy limit reached , buy_orders[0] : ', self.buy_orders[0], \
                  ' new_sell_orders[0] : ', new_sell_orders[0]
            logging.warning(log)
        # When buy_price_min is reach earlier.
                ...
                if new_buy_orders[-1][2] - new_buy_orders[0][2] \
                    ...
                    if new_buy_orders[-1][2] - self.nb_orders_to_display \
                        ...
                        i = int((self.buy_orders[0][2] - self.buy_price_min) \
                                / self.increment)
                        logging.info('buy_price_min almost reached')
    ...
    def strat(self):
        ...
        while self.don_t_touch == True:
            ...
            self.compare_orders()
            logging.info('CYCLE STOP')
            api.api_sleep()

    def main(self):
        logging.basicConfig(filename='Lazy.log', format='%(asctime)s %(levelname)s:\
            %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.WARNING)
