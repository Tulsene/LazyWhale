...
from utils.helpers import (
    interval_generator,
    get_indexes_buy_intervals,
    get_indexes_sell_intervals,
    populate_intervals,
)
...
class AnotherUserTests(TestCase):
    ...
    def setUp(self, set_root_path_patch) -> None:
        ...
        self.intervals = interval_generator(
            params["range_bot"], params["range_top"], params["increment_coef"]
        )
        ...
        self.curved_allocation = CurvedAllocation(
    ...
    def helper_amount_by_indexes(self, start_index, end_index, side):
        return sum(
            [
                self.lazy_whale.allocation.get_amount(j, side)
    ...
    def helper_test_top_is_reached_small(self):
        ...
        for i in range(self.lazy_whale.params["spread_top"], len(self.intervals)):
            orders_to_open = self.intervals[i].generate_orders_by_amount(
            ...
            self.user.create_limit_buy_order(
                self.market, orders_to_open[0]["amount"], self.intervals[i].get_top()
            )
            self.lazy_whale.main_cycle()
            self.user.create_limit_buy_order(
                self.market, orders_to_open[1]["amount"], self.intervals[i].get_top()
            )
    ...
    def helper_test_bottom_is_reached_small(self):
        ...
        for i in range(self.lazy_whale.params["spread_bot"], -1, -1):
            ...
            self.user.create_limit_sell_order(
                self.market, orders_to_open[0]["amount"], self.intervals[i].get_bottom()
            )
            self.lazy_whale.main_cycle()
            self.user.create_limit_sell_order(
                self.market, orders_to_open[1]["amount"], self.intervals[i].get_bottom()
            )
    ...
    def helper_test_top_is_reached_big(self):
        ...
        for i in range(iterations):
            amount_to_open = self.helper_amount_by_indexes(
                spr_top + 4 * i, spr_top + 4 * (i + 1), "sell"
            )

            orders_to_open = self.intervals[0].generate_orders_by_amount(
                amount_to_open + self.epsilon_amount, self.lazy_whale.min_amount, 2
            )
            self.user.create_limit_buy_order(
                self.market,
    ...
    def helper_test_bottom_is_reached_big(self):
        ...
        for i in range(iterations):
            amount_to_open = self.helper_amount_by_indexes(
                spr_bot - 4 * (i + 1) + 1, spr_bot - 4 * i + 1, "buy"
            )
    ...
    def helper_test_consume_buys_sells(self):
        ...
        def test_amount():
            if isinstance(self.lazy_whale.allocation, LinearAllocation):
                for i in range(
                    ...
                    - self.lazy_whale.params["nb_buy_to_display"]
                    + 1,
                ...
                for i in range(
                    ...
                    actual_total_amount += self.lazy_whale.intervals[
                        i
        ...
        self.user.create_limit_buy_order(
            self.market,
            self.lazy_whale.allocation.get_amount(
                self.lazy_whale.params["spread_top"], "sell"
            )
            + self.epsilon_amount,
        ...
        self.user.create_limit_sell_order(
            self.market,
            self.lazy_whale.allocation.get_amount(
                self.lazy_whale.params["spread_bot"], "buy"
            )
            + self.epsilon_amount,
        ...
        amount_to_open = self.helper_amount_by_indexes(
            self.lazy_whale.params["spread_top"],
            self.lazy_whale.params["spread_top"] + 3,
            "sell",
        )
    ...
    def helper_test_run_forever(self):
        ...
        if isinstance(self.lazy_whale.allocation, LinearAllocation) or isinstance(
        ...
        iterations = (
            len(self.intervals) // self.lazy_whale.params["nb_buy_to_display"] + 1
        )
        for i in range(iterations):
            self.assertEqual(
                self.lazy_whale.params["spread_bot"],
                len(self.intervals)
                - 4
                - self.lazy_whale.params["nb_buy_to_display"] * i,
            )
            ...
            bottom_index = max(
                ...
                len(self.intervals)
                - 4
                - self.lazy_whale.params["nb_buy_to_display"] * (i + 1)
                + 1,
            )
            amount_to_open = self.helper_amount_by_indexes(
                bottom_index,
        ...
        iterations = (
            len(self.intervals) // self.lazy_whale.params["nb_sell_to_display"] + 1
        )
        for i in range(iterations):
            self.assertEqual(
                self.lazy_whale.params["spread_top"],
    ...
    def helper_test_buy_after_startup(self):
        ...
        amount_to_open = self.helper_amount_by_indexes(
            0, self.lazy_whale.params["spread_bot"], "buy"
        )
        self.user.create_limit_sell_order(
            self.market, amount_to_open, self.intervals[3].get_bottom()
        )
    ...
    def helper_test_sell_after_startup(self):
        ...
        amount_to_open = self.helper_amount_by_indexes(
            len(self.intervals) - 8, len(self.intervals), "sell"
        )
        self.user.create_limit_buy_order(
            self.market,
            amount_to_open,
