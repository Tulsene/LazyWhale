...
class APIManagerTests(TestCase):
    ...
    def setUp(self, set_root_path_patch) -> None:
        ...
        self.api_manager.intervals = interval_generator(
            Decimal("0.01"),
            Decimal("0.015"),
        ...
        self.orders_to_open = [
            {"price": Decimal("0.0101"), "amount": Decimal("0.1")},
            {"price": Decimal("0.0102"), "amount": Decimal("0.1")},
            {"price": Decimal("0.0103"), "amount": Decimal("0.1")},
        ]
    ...
    def test_get_safety_orders(self):
        ...
        self.api_manager.create_limit_buy_order(
            self.market, Decimal("100"), self.api_manager.safety_buy_value
        )
        self.assertEqual(
            self.api_manager.get_safety_buy().price, self.api_manager.safety_buy_value
        )

        self.api_manager.create_limit_sell_order(
            self.market, Decimal("0.00001"), self.api_manager.safety_sell_value
        )
        self.assertEqual(
            self.api_manager.get_safety_sell().price, self.api_manager.safety_sell_value
        )
    ...
    def test_cancel_orders(self):
        ...
        self.assertEqual(
            len(self.api_manager.get_open_orders()), len(self.orders_to_open)
        )
